...
def generate_launch_description():
    ...
    end_effector_link_arg = DeclareLaunchArgument(
        'end_effector_link',
        default_value='tool0',
        description='End-effector link used for IK',
    )
    output_dir_arg = DeclareLaunchArgument(
        'output_dir',
        default_value='/home/sihan/ws_surface_wiping_clean/outputs',
        description='Output directory for coverage artifacts',
    )
    reachability_csv_arg = DeclareLaunchArgument(
        'reachability_csv',
        default_value='',
        description='Optional reachability CSV path',
    )
    ...
    surface_ik_server_node = TimerAction(
        period=2.0,
        actions=[
            Node(
                package='surface_wiping_nodes',
                executable='surface_ik_server',
                name='surface_ik_server',
                output='screen',
                parameters=[
